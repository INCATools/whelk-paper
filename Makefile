...
all: compare-subsumptions-all classification-time-all abox-consistency-all dl-query-speed-all
...
ontologies/%.ofn.gz: ontologies/%.owl
	robot convert -i $< -o $@.tmp.ofn && grep -v ObjectUnionOf $@.tmp.ofn | grep -v ObjectPropertyRange | grep -v ObjectHasSelf | grep -v ObjectInverse | gzip >$@
...
perf/compare-subsumptions-%.yaml: ontologies/%.ofn.gz scala/classify-compare.sc
...
perf/classification-time-$(reasoner)-$(ontology).yaml: ontologies/$(ontology).ofn.gz
	mkdir -p perf && scala-cli run scala/classify-speed-$(reasoner).sc -- ontologies/$(ontology).ofn.gz >$$@
endef

$(foreach ontology,$(ONTOLOGIES), $(foreach reasoner,$(REASONERS), $(eval $(CLASSIFICATION_SPEED_RULE))))

define QUERY_SPEED_RULE
perf/dl-query-speed-$(reasoner)-$(ontology)-$(parallelism).yaml: ontologies/$(ontology).ofn.gz
	mkdir -p perf && scala-cli run scala/dl-query-speed-$(reasoner).sc -- ontologies/$(ontology).ofn.gz $(parallelism) >$$@
endef
...
perf/abox-consistency-$(reasoner)-$(parallelism).yaml: ontologies/uberon-go-cl-ro.ofn.gz
	mkdir -p perf && scala-cli run scala/abox-consistency-$(reasoner).sc -- ontologies/uberon-go-cl-ro.ofn.gz go-cams $(parallelism) >$$@
endef
